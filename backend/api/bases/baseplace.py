...
class BasePlace(BaseAsset):
    ...
    async def get_instances(self, start_index: int = 0):
        ...
        instances_response = await self._client.requests.get(
            url=self._client.url_generator.get_url(
                "www", f"games/getgameinstancesjson"),
            params={
                "placeId": self.id,
    ...
    def get_servers(
        ...
        return PageIterator(
            ...
            url=self._client._url_generator_roblox.get_url(
                "games", f"v1/games/{self.id}/servers/{server_type.value}"),
            page_size=page_size,
            max_items=max_items,
    ...
    def get_private_servers(
        ...
        return PageIterator(
            client=self._client,
            url=self._client._url_generator_roblox.get_url(
                "games", f"v1/games/{self.id}/private-servers"),
            page_size=page_size,
            max_items=max_items,
            sort_order=sort_order,
            handler=lambda client, data: PrivateServer(
