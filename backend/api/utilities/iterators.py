...
class PageIterator(RobloxIterator):
    ...
    async def next(self):
        ...
        if page_response.status_code != 200:
            raise Exception(
                f"API request failed with status {page_response.status_code}: {page_response.text}")
        ...
        try:
            ...
            raise Exception(
                f"Failed to parse JSON response: {e}. Response content: {page_response.text[:200]}")
...
class OmniPageIterator(RobloxIterator):
    """
    An iterator wrapper for Roblox.Omni.Paging.OmniPagedResult responses.
    """
    ...
    async def next(self) -> list[Any]:
        ...
        response = await self._client.requests.cache_get(
            url=self.url,
            params={
                "pageToken": self.next_cursor if self.next_cursor else "",
                **self.extra_url_parameters
            }
        )
        response_json: dict = response.json()
        self.next_cursor = response_json.get("nextPageToken")

        page_items: list[Any] = response_json["searchResults"]
