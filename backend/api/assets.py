...
from .utilities.exceptions import AssetNotFound, BadRequest
    ...
    def __init__(self, client: Client, data: dict):
        ...
        if self.creator_type == CreatorType.user:
            self.creator: PartialUser = PartialUser(
                client=client, data=data["Creator"])
        ...
        self.icon_image: BaseAsset = BaseAsset(
            client=client, asset_id=data["IconImageAssetId"])
...
class AssetProvider:
    """
    Provides methods to interact with Roblox assets.
    """

    def __init__(self, client: Client):
        """
        Arguments:
            client: The Client to be used when getting asset information.
        """
    ...
    async def get_asset(self, asset_id: int) -> EconomyAsset:
        ...
        """
        try:
            asset_response = await self._client.requests.get(
        ...
        return EconomyAsset(client=self._client, data=asset_data)
    ...
    def get_base_asset(self, asset_id: int) -> BaseAsset:
        ...
        Returns:
            A BaseAsset.
        """
        return BaseAsset(client=self._client, asset_id=asset_id)
