...
from .utilities.exceptions import GroupNotFound, BadRequest
    ...
    async def update_shout(self, message: str, update_self: bool = True) -> Tuple[Optional[Shout], Optional[Shout]]:
        ...
        shout_response = await self._client.requests.patch(
            url=self._client.url_generator.get_url(
                "groups", f"v1/groups/{self.id}/status"),
            json={
                "message": message
            }
        )
...
class GroupProvider:
    """
    Provides methods to interact with Roblox groups.
    """

    def __init__(self, client: Client):
        """
        Arguments:
            client: The Client to be used when getting group information.
        """
        self._client: Client = client
    ...
    async def get_group(self, group_id: int) -> Group:
        ...
        """
        try:
            group_response = await self._client.requests.get(
                url=self._client.url_generator.get_url(
        ...
        return Group(client=self._client, data=group_data)
    ...
    def get_base_group(self, group_id: int) -> BaseGroup:
        ...
        Returns:
            A BaseGroup.
        """
        return BaseGroup(client=self._client, group_id=group_id)
